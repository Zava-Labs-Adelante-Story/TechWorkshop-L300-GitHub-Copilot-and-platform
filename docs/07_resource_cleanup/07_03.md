...
Remove all secrets that were created for the lab. You can do this from the command line, or from GitHub UI. 
...
Remove all variables that were created for the lab. You can do this from the command line, or from GitHub UI. 
...
You can either delete the workflow files or disable them and keep them for reference.

#### Delete Workflow Files (Recommended for Cleanup)

If you want to delete the workflow files run the following commands from the Terminal window in Visual Studio Code. Make sure you have navigated to the root of your repository first. 
...
You can disable workflows by renaming them or removing triggers. Open the YAML file and comment out the triggers:
...
You can delete individual runs from the command line or from GitHub UI.
...
You can close or delete issues created during the lab from the command line or from GitHub UI.
