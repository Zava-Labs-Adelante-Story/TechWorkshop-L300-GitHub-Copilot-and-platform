...
If the environment was provisioned using Azure Developer CLI you can use the `azd down` command to clean up all resources. 

From the Terminal window, navigate to the root of your repository and run the following command:
...
2. To ensure you are working in the correct subscription before executing the `azd down` comment, you can type `az account show` to view the current context for your Azure Developer CLI connection. The output will look similar to this:
...
<details markdown="block">
<summary><strong>Expand this section to see troubleshooting steps</strong></summary>
...
Perform the steps in this taks ONLY if:
...
From the Azure portal, navigate to the subscription you used to follow this lab and select the resource group you used for this lab. Delete the resource group. 
...
After cleanup, follow these steps verify no lab resources remain.
...
```
The resource group and associated resources used for this lab should no longer appear if the clean up is complete.
