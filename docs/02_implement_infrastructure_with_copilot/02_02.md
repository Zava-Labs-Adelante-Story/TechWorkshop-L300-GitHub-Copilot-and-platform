...
4. Prompt Copilot to generate the script
    ...
    With this command, Copilot retrieves the issue via GitHub MCP, pulls best practices from Azure MCP. It may and ask you to proceed. If you reply "yes" it will scaffold the infrastructure automatically.

    Every interaction with Copilot may produce uniqe results, however the generated files should include something like:
    ...
    Remember CoPilot is non-determinstic and may produce more files than these or leave sofe of these files out.  If you need some fo them later you can use CoPilot to generate them at that time.

### 02: Execute the Infrastructure scripts
From the command line you can first preview the results by typing the following:
...
Select the default option to scan the current directory

![azd init command requirements](../../media/0202_azd_init_2.png)

The .NET application will be detected in folder src, click enter to ```Confirm and continue initializing my app``` and next confirm the port

![azd init command requirements](../../media/0202_azd_init_2.png)

After choosing the port, the ```azure.yaml``` and the ```next-step.md``` files will be generated with instructions for the next step

![azd init command requirements](../../media/0202_azd_init_3.png)

After the initialization is finished, run the provision command with preview option to validate the created resources

```bash
azd provision --preview
```
If its the first time to run the provision command, you will be asked to provide a unique environment name 

![azd provision environment](../../media/0202_azd_provision_1.png)

If az command is authenticated, it will retreive all the available azure subscriptions for you to check one of them

![azd provision subscription](../../media/0202_azd_provision_2.png)

Next, a list of existing resource groups will be listed to use one of them, or you can create a new one

![azd provision resource group](../../media/0202_azd_provision_3.png)

Next, will choose the region for your resource group (in case of a new resource group)

![azd provision location](../../media/0202_azd_provision_4.png)

Next, provide the resource group name (in case of a new resource group)

![azd provision location](../../media/0202_azd_provision_5.png)

Finally, the provisioning will start according to the provided information and the result will be the list of resources that will be created on Azure as follow
